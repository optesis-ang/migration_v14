...
class OptesisAsset(models.Model):
    ...
    def write(self,values):
        ...
        if override_write:                                  
            resultat = self.env['optesis.etat.cel'].search([('asset_id', '=', self.code_bar)])
            _logger.info('test anta res yes we can %s',resultat.base_taxable)
            
            if self.is_taxable == True: 
                if self.id == resultat.asset_id:
                    _logger.info('test anta res yes we can %s',resultat.base_taxable)
                    self.env['optesis.etat.cel'].write({
                    'asset_id':self.id,
...
class EtatCel(models.Model):
    ...
    valeur_brute = fields.Float(related="asset_id.value",string="Valeur Brute")
    category_id = fields.Char(string="Famille")
    is_taxable = fields.Boolean('Taxable')
    base_taxable = fields.Float(string='Base Taxable',related="asset_id.base_taxable",)
    propriete = fields.Boolean(string="Propriété",related="asset_id.propriete",)
    cel_vel = fields.Float(string='CEL VEL',related="asset_id.cel_vel",)
    site = fields.Many2one(comodel_name="optesis.site", related="asset_id.site",)
    address = fields.Char(String="Addresse",related="site.address",)
    nnicad = fields.Char(String="N° NICAD",related="site.description",)
    taux_cel_vel = fields.Float("Taux Cel VL",related="asset_id.taux_cel_vel")
